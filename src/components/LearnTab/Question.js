...
export default function Question({ navigation, route }) {
    ...
    useEffect(async () => {
        await All_Questions()
            .then(async (res) => {
                setQuestionLength(res.data.questions.length)
            })
            .catch(error => {
    ...
    async function fetchingQuestions(quesNo) {
        await All_Questions()
            .then(async (res) => {
                setAllQuestions(res.data.questions[quesNo])
    ...
    return (
        <View style={styles.container}>
                        ...
                        <ScrollView>
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 20 }}>
                                {userAnswer !== `${allQuestions.Option1}` ?
                                    ...
                                    :
                                    <Text style={[styles.optionsStyle, { backgroundColor: '#c7c3c3', color: '#c7c3c3' }]} onPress={() => (replaceBlanks(`${allQuestions.Question}`, '______'), setUserAnswer('______'))}>{allQuestions.Option1}</Text>
                                }
                                {userAnswer !== `${allQuestions.Option2}` ?
                                    ...
                                    :
                                    <Text style={[styles.optionsStyle, { backgroundColor: '#c7c3c3', color: '#c7c3c3' }]} onPress={() => (replaceBlanks(`${allQuestions.Question}`, '______'), setUserAnswer('______'))}>{allQuestions.Option2}</Text>
                                }
                                {userAnswer !== `${allQuestions.Option3}` ?
                                    ...
                                    :
                                    <Text style={[styles.optionsStyle, { backgroundColor: '#c7c3c3', color: '#c7c3c3' }]} onPress={() => (replaceBlanks(`${allQuestions.Question}`, '______'), setUserAnswer('______'))}>{allQuestions.Option3}</Text>
                                }
                                {userAnswer !== `${allQuestions.Option4}` ?
                                    ...
                                    :
                                    <Text style={[styles.optionsStyle, { backgroundColor: '#c7c3c3', color: '#c7c3c3' }]} onPress={() => (replaceBlanks(`${allQuestions.Question}`, '______'), setUserAnswer('______'))}>{allQuestions.Option4}</Text>
                                }
                            </View>
                        </ScrollView>
                    }
            ...
            <ScrollView>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 15, marginBottom: 90 }}>
                    ...
                    checkAnswer ?
                            ...
                            {userAnswer === `${allQuestions.correctAns}` ?
                                <View style={{ backgroundColor: '#84e67e', padding: 10, borderRadius: 10 }}>
                                    ...
                                    {(lastQuestion === (questionNO + 1)) ?
                                        <Button onPress={() => (setUserAnswer(""), setCheckAnswer(false), navigation.navigate("ScoreCardScreen", { yourScore: yourScore + 1, skipQuestion: skipQuestion, TotalQues: questionLength }))} title="Show Score Card" color="#3e9139" />
                                        :
                                        <Button onPress={() => (setUserAnswer(""), setCheckAnswer(false), setYourScore(yourScore + 1), navigation.navigate("Question", { questionNOParams: `${Number(questionNO + 1)}` }))} title="Continue" color="#3e9139" />
                                    }
                                </View>
                                :
                                <View style={{ backgroundColor: '#e69975', padding: 10, borderRadius: 10 }}>
                                    ...
                                    {(lastQuestion === (questionNO + 1)) ?
                                        <Button onPress={() => (setUserAnswer(""), setCheckAnswer(false), navigation.navigate("ScoreCardScreen", { yourScore: yourScore, skipQuestion: skipQuestion, TotalQues: questionLength }))} title="Show Score Card" color="#BB2D3B" />
                                        :
                        ...
                        <View style={styles.row}>
                            <View style={styles.column5}>
                                {(lastQuestion === (questionNO + 1)) ?
                                    <TouchableHighlight style={styles.skipBtn} onPress={() => (setUserAnswer(""), setCheckAnswer(false), navigation.navigate("ScoreCardScreen", { yourScore: yourScore, skipQuestion: skipQuestion + 1, TotalQues: questionLength }))}>
                                        <Text style={styles.skipBtnText}>SKIP</Text>
                                    </TouchableHighlight>
                                    :
                                    <TouchableHighlight style={styles.skipBtn} onPress={() => (setUserAnswer(""), setSkipQuestion(skipQuestion + 1), setCheckAnswer(false), navigation.navigate("Question", { questionNOParams: `${Number(questionNO + 1)}` }))}>
                                        <Text style={styles.skipBtnText}>SKIP</Text>
                                    </TouchableHighlight>
                                }
